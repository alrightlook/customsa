...
CHAR_WALKRET CHAR_walk(int index, int dir, int mode) {
  ...
  if(prewalk != NULL) if(prewalk(index, &dir, &mode) == FALSE) {
...
void CHAR_walk_start(int index, int x, int y, char *dir, int mapsendmode) {
  ...
  */
  ...
  if(mapsendmode) {
    ...
    CHAR_sendMapAtWalk(index, CHAR_getInt(index, CHAR_FLOOR), CHAR_getInt(index, CHAR_X), CHAR_getInt(index, CHAR_Y), ox, oy);
  }
