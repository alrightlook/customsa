...
int CHAR_setCharHaveTitle(int charaindex, int tindex, int new) {
  ...
  if(!CHAR_CHECKINDEX(charaindex))return 0;
  if(!CHAR_CHECKTITLEINDEX(tindex))return 0;
  if(new != -1) if(TITLE_getTitleIndex(new) == -1) return 0;
...
int CHAR_DelItem(int charaindex, int ti) {
...
int CHAR_AddGold(int charaindex, int gold) {
  char token[256];
  int MyGold, MaxGold;
...
int CHAR_DelGold(int charaindex, int gold) {
  char token[256];
  int MyGold, MaxGold;
